...
class Stream:
    ...
    def add_packet(self, packet):
        ...
        packet_information.append(0. if packet[IPv6].src == self.src else 1.)
        # 负载部分
...
def read_pcap(streams, pcapname, pos=1, neg=0, cnt=10000, isVPN=False):
    try:
...
def pcap_proc(filename='test.pcap', pos=1, neg=0, cnt=10000, isVPN=False):
    pcapname = filename
    streams = {}
    read_pcap(streams, pcapname, pos, neg, cnt, isVPN=isVPN)
    return streams
