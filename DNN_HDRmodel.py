...
class traffic_DNN_HDR(nn.Module):
    def __init__(self, target=3):
        super(traffic_DNN_HDR, self).__init__()
    ...
    def forward(self, hdr, pay):
        ...
        pred = self.TS_App2(hdr)
        pred = pred.squeeze()
        # pred = nn.functional.log_softmax(pred, dim=1)
        return pred, hdr, pay


class Client_DNN_HDR:
    def __init__(
            self,
            client_id: int,
            sets: list,
        ...
        self.model = traffic_DNN_HDR(tarnum).to(device)
        self.id = client_id
